...
func TestRetrierMaxAttempts(t *testing.T) {
	...
	result := NewRetryer[int, error]().Backoff(func(ctx context.Context, attempt int, _ error) {
		j++
	}).MaxAttempts(MaxAttempts, 3).Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
		i++
		if i > 50 {
			panic("infinite loop")
		}
		return resultext.Err[int, error](io.EOF)
	})
...
func TestRetrierMaxAttemptsNonRetryable(t *testing.T) {
	...
	}).MaxAttempts(MaxAttemptsNonRetryable, 3).Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
		i++
		if i > 50 {
			panic("infinite loop")
		}
		return resultext.Err[int, error](returnErr)
	})
	Equal(t, result.IsErr(), true)
	Equal(t, result.Err(), io.EOF)
	Equal(t, i, 13)
...
func TestRetrierMaxAttemptsNonRetryableReset(t *testing.T) {
	...
	}).MaxAttempts(MaxAttemptsNonRetryableReset, 3).Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
		i++
		if i > 50 {
			panic("infinite loop")
		}
...
func TestRetrierMaxAttemptsUnlimited(t *testing.T) {
	...
	PanicMatches(t, func() {
		r.Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
			i++
			if i > 50 {
				panic("infinite loop")
			}
...
func TestRetrierMaxAttemptsTimeout(t *testing.T) {
	result := NewRetryer[int, error]().Backoff(func(ctx context.Context, attempt int, _ error) {
		...
		Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
			select {
			case <-ctx.Done():
				return resultext.Err[int, error](ctx.Err())
...
func TestRetrierEarlyReturn(t *testing.T) {
	...
	result := r.Do(context.Background(), func(ctx context.Context) resultext.Result[int, error] {
		return resultext.Err[int, error](io.EOF)
	})
	Equal(t, result.IsErr(), true)
	Equal(t, result.Err(), io.EOF)
